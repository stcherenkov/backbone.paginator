  ...
  */
  var PageableCollection = Backbone.PageableCollection = Backbone.Collection.extend({
    ...
    */
    ...
    /**
       The container object to define store strategy. These values could be used
       in "infinite" mode only.

       @property {Object} store

       @property {number} [store.offset=0] Starting full collection offset. 

       @property {number} [store.length=Infinity] Full collection size would not exceed
       this value.
    */
    store: {
      offset: 0,
    ...
    */
    constructor: function (models, options) {
      ...
      this.store = _extend({}, PageableProto.store, this.store,
                                       options.store || {});
    ...
    */
    getPage: function (index, options) {
      ...
      var storeOffset = this.store && this.store.offset || 0;

      var pageStart = (firstPage === 0 ? pageNum : pageNum - 1) * pageSize;
      var pageModelsStart = pageStart - storeOffset;
      var pageModelsEnd = pageStart + pageSize - storeOffset;
      var pageModels = fullCollection && fullCollection.length && (!storeOffset || pageStart >= storeOffset) ?
        fullCollection.models.slice(pageModelsStart, pageModelsEnd) :
        [];
      ...
      if (mode != "server") {
        ...
        options.success = function (col, resp, opts) {
          ...
          if (mode == "client") fullCol.reset(models, opts);
            ...
            var forward = (options.to >= options.from);
            if (fullCol.size() + models.length > store.length) {
              var modelsToRemove = forward ?
                fullCol.first(models.length) :
                fullCol.last(models.length);
              _each(modelsToRemove, function (model) {
                fullCol.remove(model, {onAdd: true});
              });
              store.offset += (forward ? models.length : - models.length);
            }
            fullCol.add(models, _extend({at: forward ? fullCol.length : 0},
